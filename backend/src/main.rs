...
async fn expedition_handler(State(state): State<AppState>,Path(id): Path<Uuid>) -> Response {
...
async fn rename_planet_handler(State(state): State<AppState>,Path(id): Path<Uuid>, Json(payload): Json<RenamePlanetPayload>) -> impl IntoResponse {
