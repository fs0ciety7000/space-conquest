...
mod handlers;
use handlers::fleet::{
    load_planet_ships_for_combat, load_planet_tech_bonuses, load_planet_defenses_for_combat,
    load_zac_ships_for_combat,
    set_planet_ships, set_planet_defenses_after_combat, add_ships_to_planet,
};
...
async fn colonize_handler(
    ...
    // Valider que la source a assez pour le supplément uniquement (la base est gratuite)
    if metal_to_transport > att_planet_data.metal_amount {
        return (StatusCode::BAD_REQUEST, Json(json!({"error": format!("Pas assez de métal sur la planète source ({:.0} disponible, {:.0} demandé).", att_planet_data.metal_amount, metal_to_transport)}))).into_response();
    }
    if crystal_to_transport > att_planet_data.crystal_amount {
        return (StatusCode::BAD_REQUEST, Json(json!({"error": format!("Pas assez de cristal sur la planète source ({:.0} disponible, {:.0} demandé).", att_planet_data.crystal_amount, crystal_to_transport)}))).into_response();
    }
    if deuterium_to_transport > att_planet_data.deuterium_amount {
        return (StatusCode::BAD_REQUEST, Json(json!({"error": format!("Pas assez de deutérium sur la planète source ({:.0} disponible, {:.0} demandé).", att_planet_data.deuterium_amount, deuterium_to_transport)}))).into_response();
    }
