...
export default function MessagesView({ token, userId, initialRecipient }: MessagesViewProps) {
    ...
    const fetchMessages = async () => {
        setLoading(true);
        try {
            const res = await fetch(`http://localhost:8080/messages?user_id=${userId}`, {
    ...
    const handleMarkRead = async (id: string) => {
        try {
             await fetch(`http://localhost:8080/messages/${id}/read`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` }
            });
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        setIsSending(true);
        try {
            const res = await fetch(`http://localhost:8080/messages/send?user_id=${userId}`, {
                method: 'POST',
