...
export default function GalaxyView({ planet, onNavigateAttack, onNavigateSpy, onNavigateTransport }: GalaxyViewProps) {
    ...
    const fetchSystem = async () => {
        setLoading(true);
        try {
            const res = await fetch(`/galaxy/${galaxy}/${system}?current_planet_id=${planet.id}`);
    ...
    const handleColonize = async (position: number) => {
        ...
        try {
            const res = await fetch(`/colonize?current_planet_id=${planet.id}`, {
                method: 'POST',
    ...
    const handleRecycle = async (targetId: string) => {
        const token = localStorage.getItem('token');
        try {
            const res = await fetch(`/recycle?current_planet_id=${planet.id}`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
