...
export default function UniversalProfile({ userId, onClose, isModal = false }: UniversalProfileProps) {
  ...
  const handleAcceptRequest = async () => {
    ...
    setFriendLoading(true);
    try {
      const res = await fetch(apiUrl(`/friends/${friendStatus.friendship_id}/accept?user_id=${viewerId}`), {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const handleRemoveFriend = async () => {
    ...
    setFriendLoading(true);
    try {
      const res = await fetch(apiUrl(`/friends/${friendStatus.friendship_id}?user_id=${viewerId}`), {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const getRankColor = (badge: string) => {
    if (!badge || badge === 'CLASSIFIÉ') return "from-red-500 to-red-700";
  ...
  const handleSaveBio = async () => {
    ...
    try {
      const res = await fetch(apiUrl(`/users/${userId}/bio`), {
        method: "PUT",
        headers: { "Content-Type": "application/json", 'Authorization': `Bearer ${localStorage.getItem('token')}` },
        body: JSON.stringify({ bio: bioInput || null }),
      });
      if (res.ok) {
        setProfile((prev: any) => ({ ...prev, bio: bioInput }));
        setEditingBio(false);
  ...
  const content = (
    <div className={`max-w-5xl w-full max-h-[90vh] overflow-y-auto bg-slate-950 border border-white/10 rounded-3xl shadow-2xl ${!isModal ? 'mx-auto' : ''}`}>
      {/* Header */}
      ...
      <div className="relative px-8 pb-8">
        <div className="flex flex-col md:flex-row items-center md:items-end gap-6 -mt-16 mb-6">
          ...
          <div className="text-center bg-gradient-to-br from-indigo-950 to-purple-950 px-6 py-4 rounded-2xl border border-indigo-500/30 shadow-lg">
            <div className={`text-3xl font-black font-mono flex items-center gap-2 justify-center ${isClassified(profile.total_points) ? 'text-red-400' : 'text-white'}`}>
              ...
              {typeof profile.total_points === 'number' ? profile.total_points.toLocaleString() : profile.total_points}
            </div>
          ...
          <CardContent className="p-4">
            ...
            {editingBio ? (
              <div>
                ...
                />
                <div className="text-right text-xs text-slate-500 mt-1">{bioInput.length}/500</div>
              </div>
            ) : (
              <p className="text-slate-300 text-sm leading-relaxed">
                {profile?.bio || <span className="text-slate-500 italic">Aucune biographie.</span>}
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <StatCard icon={Globe} label="Planètes" value={profile.planet_count} color="text-blue-400" classified={false} />
          <StatCard icon={Rocket} label="Flotte" value={profile.total_fleet} color="text-purple-400" classified={isClassified(profile.total_fleet)} />
          <StatCard icon={Shield} label="Défenses" value={profile.total_defenses} color="text-red-400" classified={isClassified(profile.total_defenses)} />
...
function StatCard({ icon: Icon, label, value, color, classified }: { icon: any, label: string, value: string | number | undefined, color: string, classified: boolean }) {
  return (
    <div className="bg-slate-900/50 border border-white/5 rounded-xl p-4 text-center overflow-hidden">
...
function BuildingLevel({ label, level }: { label: string, level: number | string | undefined }) {
  ...
  return (
    <div className="flex justify-between items-center text-xs">
      ...
      <span className={`font-mono font-bold flex items-center gap-1 ${isClassified ? 'text-red-400' : 'text-white'}`}>
        ...
        {isClassified ? level : (level !== undefined ? `Niv. ${level}` : '—')}
      </span>
    </div>
  );
}
