...
export default function Shipyard({ planet, onUpdate }: ShipyardProps) {
  ...
  const maxFleet = 500 + ((planet.hangar_level || 0) * 500); // Formule Backend
  ...
  const buildShip = async (type: string) => {
    ...
    // Vérification client-side pour éviter l'appel API inutile
    if (amount > remainingSpace) {
        toast.error("Capacité du hangar insuffisante !");
        return;
    }
    ...
    try {
        ...
        if(res.ok) {
            ...
            // Gestion du code erreur 409 Conflict (Hangar plein)
            if (res.status === 409) {
                toast.error("Opération annulée", { description: "Le hangar est plein." });
            } else {
  ...
  return (
    <div className="space-y-6 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
         ...
         <CardContent className="p-4 flex items-center justify-between relative z-10">
            ...
            <div className="flex-1 mx-8">
                ...
                <div className="h-2 w-full bg-slate-800 rounded-full overflow-hidden">
                    <div 
                        className={`h-full transition-all duration-1000 ${isFull ? 'bg-red-500 animate-pulse' : 'bg-orange-500'}`} 
      ...
      {fleet.map((ship) => {
        ...
        // Calcul du Max Constructible : Min(Ressources, Espace Hangar)
        ...
        const maxBuildable = Math.min(maxMetal, maxCrystal, remainingSpace); // <-- Ajout remainingSpace
            ...
            <CardContent className="p-5 relative z-10 flex flex-col h-full">
              ...
              <div className="mt-auto">
                <div className="flex justify-between text-[10px] mb-1 px-1">
                   ...
                   <button 
                      onClick={() => !locked && !isShipyardBusy && !isFull && setQty({...qty, [ship.id]: maxBuildable})}
                ...
                <div className="flex gap-2">
                    <input 
                        ...
                        disabled={locked || isShipyardBusy || isFull}
                    />
                    <Button 
                        onClick={() => buildShip(ship.id)}
                        disabled={locked || isShipyardBusy || !canAfford || (qty[ship.id] || 0) <= 0 || isFull}
                    ...
                    >
                        {!isShipyardBusy && canAfford && !isFull && (
