...
import { 
  ...
  Hammer, Crosshair, Truck, Box, Zap, Scan, Sword, Warehouse 
} from "lucide-react";
...
export default function Shipyard({ planet, onUpdate }: ShipyardProps) {
  ...
  const fleet = [
    { id: 'light_hunter', name: 'Chasseur Léger', cost: { m: 3000, c: 1000 }, stats: { atk: 50, def: 10, fret: 50 }, icon: Crosshair, type: 'OFFENSIF', class: 'Intercepteur', desc: "Vaisseau d'attaque rapide." },
    { id: 'cruiser', name: 'Croiseur', cost: { m: 20000, c: 7000 }, stats: { atk: 400, def: 50, fret: 800 }, icon: Shield, type: 'OFFENSIF', class: 'Frégate Lourde', desc: "Blindé, tueur de chasseurs." },
    { id: 'transporter', name: 'Transporteur', cost: { m: 4000, c: 2000 }, stats: { atk: 5, def: 10, fret: 5000 }, icon: Truck, type: 'LOGISTIQUE', class: 'Cargo Standard', desc: "Indispensable pour le fret." },
    { id: 'colony_ship', name: 'Vaisseau Colon', cost: { m: 10000, c: 20000 }, stats: { atk: 50, def: 100, fret: 7500 }, icon: Rocket, type: 'LOGISTIQUE', class: 'Module Arche', desc: "Fonde de nouvelles colonies." },
    { id: 'recycler', name: 'Recycleur', cost: { m: 10000, c: 6000 }, stats: { atk: 1, def: 10, fret: 20000 }, icon: Hammer, type: 'UTILITAIRE', class: 'Collecteur', desc: "Récolte les débris spatiaux." },
    { id: 'spy_probe', name: 'Sonde Espion', cost: { m: 0, c: 1000 }, stats: { atk: 0, def: 1, fret: 5 }, icon: Info, type: 'RENSEIGNEMENT', class: 'Drone Furtif', desc: "Scanner longue portée." },
  ];
...
function StatBox({ icon: Icon, label, value, color }: { icon: any, label: string, value: number, color: string }) {
    return (
        <div className="flex flex-col items-center bg-black/40 border border-white/5 rounded p-2">
            <span className={`text-[9px] font-black uppercase ${color} flex items-center gap-1 mb-1`}><Icon size={10} /> {label}</span>
