...
// Speed factor utilisé par le serveur pour calculer les récompenses
const SPEED_FACTOR = 5.0; // 500 / 100 = 5.0x
              ...
              <div className="text-right">
                ...
                <div className="space-y-1 text-xs mb-3">
                  <div className="text-slate-300">
                    +{Math.floor(overview.streak.next_reward.metal * SPEED_FACTOR).toLocaleString()} <span className="text-slate-500">métal</span>
                  </div>
                  <div className="text-slate-300">
                    +{Math.floor(overview.streak.next_reward.crystal * SPEED_FACTOR).toLocaleString()} <span className="text-slate-500">cristal</span>
                  </div>
                </div>
                <Button
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex gap-4 mt-3 text-[10px] text-slate-500">
                          {mission.reward_metal > 0 && (
                            <span>+{Math.floor(mission.reward_metal * SPEED_FACTOR * (overview?.streak.streak_bonus_multiplier || 1)).toLocaleString()} métal</span>
                          )}
                          {mission.reward_crystal > 0 && (
                            <span>+{Math.floor(mission.reward_crystal * SPEED_FACTOR * (overview?.streak.streak_bonus_multiplier || 1)).toLocaleString()} cristal</span>
                          )}
                          {mission.reward_deuterium > 0 && (
                            <span>+{Math.floor(mission.reward_deuterium * SPEED_FACTOR * (overview?.streak.streak_bonus_multiplier || 1)).toLocaleString()} deutérium</span>
                          )}
                          {mission.reward_xp > 0 && (
